...
Store task-local state beside the active task contract. Update project-level documents only through
`$qta-delivery-finalization`, never from an implementation checkpoint.
...
- Do not update project-level documents (`README.md`, `AGENTS.md`, `CLAUDE.md`) outside finalization.
