...
beforeEach(() => {
  clearAll();
  // 锁定 mock 模式，避免依赖后端。pick() 每次现读 settings，这里固定后测试内一致。
  saveSettings({ apiMode: 'mock', apiBaseUrl: '' });
});

describe('tradeJournalApi', () => {
  it('新增交易记录自动计算 amount', async () => {
  ...
  it('更新 price 后重新计算 amount', async () => {
    const journal = await addTradeJournal({
      tradeDate: '2026-06-08',
      symbol: '300750',
      side: 'BUY',
  ...
  it('更新 quantity 后重新计算 amount', async () => {
    const journal = await addTradeJournal({
      tradeDate: '2026-06-08',
      symbol: '300750',
      side: 'SELL',
    ...
    const updated = await updateTradeJournal(journal.id, { quantity: 200 });
    expect(updated?.amount).toBeCloseTo(10000, 2);
  });

  it('updateReviewStatus 单条更新', async () => {
    const journal = await addTradeJournal({
      tradeDate: '2026-06-08',
      symbol: '300750',
      side: 'BUY',
    ...
    const updated = await updateReviewStatus(journal.id, 'REVIEWED');
    expect(updated?.reviewStatus).toBe('REVIEWED');
  });

  it('batchUpdateReviewStatus 批量更新', async () => {
    const j1 = await addTradeJournal({ tradeDate: '2026-06-08', symbol: 'A', side: 'BUY', price: 10, quantity: 100, emotionTags: [], mistakeTags: [] });
    const j2 = await addTradeJournal({ tradeDate: '2026-06-08', symbol: 'B', side: 'SELL', price: 20, quantity: 100, emotionTags: [], mistakeTags: [] });
    const j3 = await addTradeJournal({ tradeDate: '2026-06-08', symbol: 'C', side: 'BUY', price: 30, quantity: 100, emotionTags: [], mistakeTags: [] });

    await batchUpdateReviewStatus([j1.id, j3.id], 'REVIEWED');
  ...
  it('新增带费用字段并透传保存，amount 仍为毛额不含费用', async () => {
    const journal = await addTradeJournal({
      tradeDate: '2026-06-08',
      symbol: '300750',
      side: 'BUY',
  ...
  it('更新费用字段', async () => {
    ...
    const updated = await updateTradeJournal(journal.id, { commissionFee: 10, stampTax: 2 });
