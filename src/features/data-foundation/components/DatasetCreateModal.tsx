...
export interface DatasetCreateModalProps {
  ...
  /** 默认 provider（如导入面板预选 IMPORT_CSV_DAILY）。 */
  defaultProviderCode?: string;
}

export function DatasetCreateModal({ open, onClose, onCreated, defaultProviderCode }: DatasetCreateModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      if (defaultProviderCode) {
        form.setFieldValue('providerCode', defaultProviderCode);
      }
    }
  }, [open, defaultProviderCode, form]);
  ...
  return (
    ...
    >
      <Form<FormValues> form={form} layout="vertical" onFinish={onFinish}>
        ...
        <Form.Item name="marketCode" label="市场（首期固定 CN）" initialValue="CN">
          <Select disabled options={[{ value: 'CN', label: 'CN' }]} data-testid="dataset-create-market" />
        </Form.Item>

        <Form.Item name="barType" label="K 线类型（首期固定 DAILY）" initialValue="DAILY">
          <Select disabled options={[{ value: 'DAILY', label: 'DAILY' }]} data-testid="dataset-create-bartype" />
        </Form.Item>

        <Form.Item name="frequency" label="频率（首期固定 1D）" initialValue="1D">
          <Select disabled options={[{ value: '1D', label: '1D' }]} data-testid="dataset-create-frequency" />
        </Form.Item>

        <Form.Item
          name="providerCode"
          label="Provider（首期仅以下组合，导入类数据集选 IMPORT_CSV_DAILY）"
          rules={[{ required: true, message: '请选择 Provider' }]}
        >
          <Select
            options={DATASET_PROVIDER_OPTIONS}
