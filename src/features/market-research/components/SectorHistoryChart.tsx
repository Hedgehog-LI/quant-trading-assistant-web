...
export function SectorHistoryChart({ points, oneDay = false }: Props) {
  if (points.length === 0) {
    return <Empty image={Empty.PRESENTED_IMAGE_SIMPLE}
      description={oneDay ? '暂无一日收盘强度历史' : '暂无已发布历史'} />;
  }
  const width = 760;
  const height = 300;
  const left = 48;
  ...
  const ordered = [...points].sort((leftPoint, rightPoint) =>
    leftPoint.asOfDate.localeCompare(rightPoint.asOfDate));
  ...
  return (
    <div className="sector-history-chart" data-testid="sector-history-chart">
      <svg viewBox={`0 0 ${width} ${height}`} role="img"
        aria-label={oneDay ? '板块每日强度历史轨迹' : '板块相对强弱历史轨迹'}>
        ...
        {ordered.map((point, index) => {
          const p = position(point, index);
          return (
            <g key={`${point.sourceBatchId ?? point.publicationBatchId}-${point.asOfDate}`}>
