  ...
  return {
    ...
    contract: {
      ...
      implementationSlices: [{
        ...
        allowedWritePaths: ["src/features/example"], maxExpectedFiles: 4, maxProductionLineDelta: 300
      }],
      testInventory: [
...
test("architecture gate flags an oversized frontend module", () => {
  const methods = Array.from({ length: 31 }, (_, index) =>
    `  function method${index}() { axios.get("/api"); }`).join("\n");
  const filler = Array.from({ length: 620 }, () => "  const value = 1;").join("\n");
  const source = `import axios from "axios";\n${methods}\n${filler}`;
  const report = analyzeSource("src/features/demo/GridAdapter.ts", source);
  assert.ok(report.errors.some((error) => error.includes("class/module")));
});

test("architecture parser detects multiline long methods", () => {
  const body = Array.from({ length: 110 }, (_, index) => `    const value${index} = ${index};`).join("\n");
  const source = `export function calculate(input: number): number {\n${body}\n  return input;\n}`;
...
test("architecture gate accepts a small typed module", () => {
  const source = "export function add(left: number, right: number) { return left + right; }";
  assert.deepEqual(analyzeSource("src/features/demo/add.ts", source).errors, []);
});
...
test("architecture CLI analyzes the exact frozen SNAPSHOT manifest", async () => {
  ...
  const sourcePath = "src/features/demo/SnapshotGrid.tsx";
  try {
    await mkdir(path.join(directory, path.dirname(sourcePath)), { recursive: true });
    await writeFile(path.join(directory, sourcePath), "export function SnapshotGrid() { return null; }\n");
    for (const args of [["init", "-q"], ["config", "user.email", "qta@example.test"],
    ...
    assert.match(result.stdout, /SnapshotGrid\.tsx/);
    assert.match(result.stdout, /class\/module/);
...
function tsMethodBodyFixture(lineCount) {
  const lines = ["export function calculate(input: number): number {", "  let total = input;"];
  for (let index = 0; index < lineCount; index += 1) lines.push(`  total += ${index};`);
  lines.push("  return total;");
  lines.push("}");
  return `${lines.join("\n")}\n`;
}

test("baseline-aware gate still blocks over-long module methods", async () => {
  const candidateDir = await mkdtemp(path.join(os.tmpdir(), "qta-baseline-long-method-cand-"));
  const baselineDir = await mkdtemp(path.join(os.tmpdir(), "qta-baseline-long-method-base-"));
  try {
    const relativeFile = "src/features/demo/LongMethod.ts";
    await mkdir(path.join(baselineDir, path.dirname(relativeFile)), { recursive: true });
    await mkdir(path.join(candidateDir, path.dirname(relativeFile)), { recursive: true });
    await writeFile(path.join(baselineDir, relativeFile), tsMethodBodyFixture(95));
    await writeFile(path.join(candidateDir, relativeFile), tsMethodBodyFixture(120));
    const reportPath = path.join(candidateDir, "architecture-report.json");
    const result = await runBaselineAwareGate({
      candidateDir, baselineDir, candidateFile: relativeFile, reportPath
    });
